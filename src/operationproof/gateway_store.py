...
class GatewayAdmissionStore(ABC):
    """Atomic replay boundary for gateway admission proofs and one-time tokens."""

    @abstractmethod
    def reserve(self, record: GatewayAdmissionRecord) -> str:
        """Reserve one proof digest exactly once and return an opaque admission token."""
...
class MemoryGatewayAdmissionStore(GatewayAdmissionStore):
    ...
    Proof digests are retained for the lifetime of the process to prevent re-admission.
    Capacity exhaustion fails closed instead of evicting replay history.
    """
    ...
    def reserve(self, record: GatewayAdmissionRecord) -> str:
        ...
        with self._lock:
            ...
            if len(self._reserved_proofs) >= self._max_records:
